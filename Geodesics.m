%% This MATLAB script plots symmetric geodesics in the compactified coordinates (rho,x)
...
%% Wall parametrization and geodesic plot

% The boundary rho = \pi/2
...
for i=1:2
    ...
    figure(i)
    h1 = plot(eps*x1,y1, 'g-', 'LineWidth',3);
    hold on
    plot(eps*x1,-y1, 'g-', 'LineWidth',3);
    hold on
    h5 = plot(eps*x2,y2, 'g-', 'LineWidth',3);
    hold on
    plot(eps*x2,-y2, 'g-', 'LineWidth',3);
    hold on
    fill(xb,yb,'k');
    hold on
    ...
    axis equal
    legend([h3,h1,h4], {'boundary $\rho=\frac{\pi}{2}$', 'Geodesic','wall'}, 'Interpreter','latex','Location','northwest')
